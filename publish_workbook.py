...
def get_default_project_id(server, auth_token, site_id, project_name):
    ...
    #project_name = raw_input("\nProject name that workbook to be publish(Press enter for default project): ")
    print("\n2. Finding the PROJECT ID FOR GIVEN PROJECT to publish to")
    project_name = 'default' if project_name =='' else project_name.lower()
	#project_name = project_name.lower()
    
    for project in projects:
        if project.get('name').lower() == project_name:
            print('project_name_non', project_name,project.get('name'))
            return project.get('id')
    
    raise LookupError("Project named 'default' was not found on server")


def main(workbook_file_path, server, username, password, site, project_name, db_username, db_password):
    ...
    ##### STEP 2: OBTAIN  PROJECT ID FOR GIVEN PROJECT_NAME#####
    #print("\n2. Finding the PROJECT ID FOR GIVEN PROJECT to publish to")
    ...
    ET.SubElement(workbook_element, 'connectionCredentials', name=db_username, password=db_password, embed='true', oAuth='false')
    ...
    _check_status(server_response, 201)
    req_payload_path = r"C:\Users\narendra.kommoju\Desktop\Narendra\Tableau_Python\Req_payload.txt"
    
    ##### STEP 4: SIGN OUT #####
    print("\n4. Signing out, and invalidating the authentication token")
    print("\nDone publishing workbook {0}".format(workbook_file))
...
if __name__ == '__main__':
    import csv
    print("This is console app to publish workbook.\nPlease provide the .csv or .txt file as below format.\nfilename,servername,username,password,sitename,projectname,db_username,db_password\n")
    server_detalis_file_path = raw_input("Give file name(along with path) in .csv format: ")
    server_detalis_file_path = os.path.abspath(server_detalis_file_path)
    server_detalis_file = os.path.basename(server_detalis_file_path)
    server_detalis_filename, s_file_extension = server_detalis_file.split('.', 1)
    workbooks = []
    if s_file_extension == 'txt':
        with open(server_detalis_file_path,"r") as file:
            reader = list(file.readlines())
        workbooks = [line.rstrip('\n').split(',') for line in reader]
    elif s_file_extension == 'csv':
        with open(server_detalis_file_path,"r") as file:
            reader = list(csv.reader(file, delimiter=","))
            for  line in reader:
                line = [l for l in line[0].split(',')]
                workbooks.append(line)
    else:
        print("Error: Invalid File Format")
		
    for workbook in workbooks:
        workbook[0] = os.path.abspath(workbook[0])
        print("workbook",workbook)
        workbook_file_name = os.path.basename(workbook[0])
        print("\nPublishing workbook {0} on server {1} to project {2}.".format(workbook_file_name, workbook[1], workbook[5]))
        main(workbook[0], workbook[1], workbook[2], workbook[3], workbook[4], workbook[5], workbook[6], workbook[7])
    print("\nDone publishing all workbooks in CSV file")
